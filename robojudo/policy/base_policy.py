...
class Policy(ABC):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        """Compute action with smoothing/clipping; keeps internal state as torch.Tensor."""
        obs_tensor = torch.from_numpy(obs).unsqueeze(0).float().to(self.device)
        with torch.no_grad():
            actions_tensor = self.model(obs_tensor).cpu().squeeze(0)

        # Ensure last_action is torch tensor on CPU
        if isinstance(self.last_action, np.ndarray):
            last_action_t = torch.from_numpy(self.last_action).float()
        else:
            last_action_t = self.last_action
        last_action_t = last_action_t.to(actions_tensor.device).float().reshape(-1)
        actions_tensor = actions_tensor.reshape(-1)

        # Smooth
        actions_t = (1 - self.action_beta) * last_action_t + self.action_beta * actions_tensor

        # Update state
        self.last_action = actions_t.clone()

        # Clip if needed
        if self.action_clip is not None:
            actions_t = torch.clamp(actions_t, -self.action_clip, self.action_clip)

        # Scale, flatten, and return numpy 1-D
        actions_t = actions_t * self.action_scale
        return actions_t.cpu().numpy().reshape(-1)
