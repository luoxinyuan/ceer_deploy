...
class PolicyWrapper:
    ...
    def get_pd_target(self, obs):
        ...
        # print(f'default_pos: {self.policy.default_pos[:8]}')
        # print(f'pd_target: {pd_target[:8]}')
...
class RlPipeline(Pipeline):
    ...
    def __init__(self, cfg: RlPipelineCfg):
        ...
        env_joint_names_before_override = list(self.env.dof_cfg.joint_names)
        self.env.update_dof_cfg(override_cfg=self.policy.cfg_action_dof)
        self.visualizer = self.env.visualizer

        def _format_joint_order(joint_names: list[str]) -> str:
            return "\n".join([f"  [{i:02d}] {name}" for i, name in enumerate(joint_names)])

        logger.warning(
            "Joint order snapshot (env before override):\n%s",
            _format_joint_order(env_joint_names_before_override),
        )
        logger.warning(
            "Joint order snapshot (env after override):\n%s",
            _format_joint_order(list(self.env.dof_cfg.joint_names)),
        )
        logger.warning(
            "Joint order snapshot (policy obs_dof):\n%s",
            _format_joint_order(list(self.policy.cfg_obs_dof.joint_names)),
        )
        logger.warning(
            "Joint order snapshot (policy action_dof):\n%s",
            _format_joint_order(list(self.policy.cfg_action_dof.joint_names)),
        )
    ...
    def step(self, dry_run=False):
        ...
        if not dry_run and self.visualizer is not None:
